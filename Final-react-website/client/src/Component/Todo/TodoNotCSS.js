...
export default function TodoNotCss() {
    ...
    const editStart = (index, todo) => {
        ...
        /*** 
        수정을 진행할 경우 할 일 목록에 있는 할일만 가져올 예정 
    ...
    // 수정한 내용 저장하는 버튼
    const saveEdit = () => {
        const updateTodos = [...todos];                 // 이 부분 is not a function 에러 떠서 수정
        // updateTodos[editingIndex] = {newTodo: editTodo,  date};
        // newTodo 의 경우 수정한 내용을 새로 넣어주기 때문에 값 대칭을 해준 것
        // date 같은 경우 처음부터 선택하게 만들 에정이기 때문에
        // date 만 넣어줌
        updateTodos[editingIndex] = {newTodo: editTodo,  date};
