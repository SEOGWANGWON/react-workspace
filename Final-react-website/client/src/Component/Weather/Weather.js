...
function Weather() {
    ...
    const [latitude, setLatitude] = useState(null);     // latibude = 위도
    ...
    }, []);     // 이 공백을 넣어주는 이유가 앞서 있던 함수를 못가져오면 보여줄 빈 배열;

    // 검색할 버튼 함수 생성
    const searchWeather = async () => {
        try{
            setLoading(true);
            const response = await axios.get( `https://api.weatherbit.io/v2.0/current?lat=${latitude}&lon=${longitude}&key=${API_KEY}`); //  `https://api.weatherbit.io/v2.0/current?city=${query}&key=${API_KEY}`  weatherbit.io유료사이트
