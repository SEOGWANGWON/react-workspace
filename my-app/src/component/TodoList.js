...
// class App extends Componenets {} 동일
// const App = () => {} 동일
function App() { // 동일
  ...
  const addTodo = () => {
    ...
    if(newTodo.trim() !== ''){
        // setTodos([...todos, newTodo]) 현재 입력되어 있는 할 일 목록
        // 배열인 todos를 복사해서 복사한 todos에 
        // 새로운 할일인 newTodo를 배열에 추가한 후
        // 할 일 목록 설정에 newTodo가 추가된 목록으로 최종 설정해주기 위한 setTodos

        setTodos([...todos, newTodo]);

        // 저장된 할일 목록을 초기화 시켜주기 위해 setNeWtODO를 초기화
        setNewTodo('');
    }
